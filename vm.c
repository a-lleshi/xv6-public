...
// Calling mprotect() should change the protection bits of the page range starting at addr and
//  of len pages to be read only. Thus, the program could still read the pages in this range after
//  mprotect() finishes, but a write to this region should cause a trap (and thus kill the process).
...
mprotect(void *addr, int len) {
  ...
  // If len is <1 or if addr pages not aligned is true then return false
  if (len <= 0 || (uint) addr % PGSIZE != 0) {
    return -1;
  }

  // For loop to iterate through the pages in the range
  for (int i = (uint) addr; i < ((int) len * PGSIZE + (int) addr); i += PGSIZE) {
    p = walkpgdir(myproc() -> pgdir, (void *) i, 0);
...
munprotect(void *addr, int len) {
  ...
  // Return true if all pages were successfully set to read-write & flush the TLB
  lcr3(V2P(myproc() -> pgdir));
  return 0;  
}